...
/// tab. Owns its own dial state, sort preference, and last-played selection.
struct PlaylistsView: View {
	...
	var body: some View {
		NavigationView {
			...
			// Intentionally no refetch on play: re-sorting by last-played mid-
			// session pushes the just-played playlist to index 0, and the
			// reanchor that follows mutates the wheel rotation unanimated —
			...
			.onChange(of: focusedIndex) { _, newIdx in
				...
				// reorders. Skip during spin — focused changes per frame
				// during shuffle and we set the id manually at land time.
	...
	private func applyPlaylists(_ new: MusicItemCollection<Playlist>) {
		let preservedID: MusicItemID? = focusedPlaylistID
	...
	private func reanchor(to newIdx: Int, in newPlaylists: MusicItemCollection<Playlist>) {
		...
		let cp = -rotation.degrees / DialView<Playlist>.stepVisual
		...
		if diff < -half { diff += Double(count) }
		let newCp = cp + diff
		rotation = .degrees(-newCp * DialView<Playlist>.stepVisual)
		focusedIndex = newIdx
		focusedPlaylistID = newPlaylists[newIdx].id
	}

	/// Spins the wheel to a random nearby playlist. When `autoplay` is on,
	/// also plays the landed playlist; otherwise it just lands and waits for
	/// the user to hit Play.
	func shuffle() async {
		...
		let destination = DialView<Playlist>.spinDestination(
			current: rotation,
			target: target,
			count: count
		)
		...
		let distance = abs(destination.degrees - rotation.degrees) / DialView<Playlist>.stepVisual
	...
	func play(_ playlist: Playlist) async {
		guard let detailed = try? await playlist.with([.entries]) else { return }
		await playPlaylist(detailed)
	}

	private func playPlaylist(_ playlist: Playlist) async {
		do {
			guard let firstEntry = playlist.entries?.first else { return }
